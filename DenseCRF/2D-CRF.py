...
for file_index, file in enumerate(os.listdir(para.test_liver_ct_path)):
    ...
    pred = sitk.ReadImage(os.path.join(para.pred_liver_path, file.replace('volume', 'pred')), sitk.sitkUInt8)
    pred_array = sitk.GetArrayFromImage(pred)

    seg = sitk.ReadImage(os.path.join(para.test_liver_seg_path, file.replace('volume', 'liver-segmentation')), sitk.sitkUInt8)
...
writer = pd.ExcelWriter('./result_pred_xlsx/result-post-processing_five_five.xlsx')
